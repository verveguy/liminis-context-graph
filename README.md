...
**Size contract**: extraction quality degrades sublinearly as `chunk_text` grows, so keep chunks near the advisory threshold (`LCG_CHUNK_TEXT_ADVISORY_MAX_CHARS`, default 8,000 characters) rather than passing whole documents. `chunk_text` above the threshold is split internally into threshold-sized units, each preferring a whitespace boundary over a hard mid-word cut; every unit is ingested as its own episode, and all of them share the caller's `chunk_id`. When a split actually runs, the response carries `episode_uuids` (array) and `unit_count` instead of a single `episode_uuid`, plus a `warning` object naming the actual size and the threshold — but an idempotent no-op resubmission (see below) skips extraction/splitting entirely and reuses whatever shape the existing episode(s) already have — e.g. a chunk_id ingested as a single episode still returns a singular `episode_uuid` on a no-op, even carrying a `warning` if the threshold has since been lowered, since a no-op never retroactively re-splits stored data. `knowledge_delete_chunk_episode` deletes every unit sharing a `chunk_id`, split or not.

**Resubmission idempotency**: calling `knowledge_process_chunk` again with a `chunk_id` you've already ingested is deterministic, not additive. Identical `chunk_text` is a no-op — extraction is skipped entirely and the existing episode(s) are returned with `idempotent: true`, avoiding re-running the (nondeterministic) extraction LLM on a byte-identical retry. Different `chunk_text` replaces the prior episode(s) for that `chunk_id` — the fresh ingest runs first, and the prior episode(s) are deleted only after it succeeds, so a failed re-ingest never leaves the `chunk_id` with zero episodes — and reports what was deleted in `replaced_uuids`. This applies the same way whether the chunk_id maps to a single episode or a split chunk's multiple units — a `chunk_id` never silently accumulates unrelated episodes across calls. This guarantee holds for **serialized** calls only: two calls for the *same* `chunk_id` in flight at once — whether a client retry racing the original in-flight request, or two independent callers happening to submit the same `chunk_id` concurrently for the first time — are not mutually exclusive and can both insert, producing duplicate episodes that only get cleaned up on a later, non-concurrent resubmission. Callers wanting exactly-once semantics should serialize calls per `chunk_id` themselves rather than firing them concurrently. `chunk_id` matching is also independent of `source_file` (so idempotency survives a document rename or move) and is scoped only by `group_id` — treat `chunk_id` as required to be unique per logical document within a `group_id`, not merely unique per `source_file`, since two unrelated documents that reuse the same `chunk_id` are treated as one lineage and can replace each other's episodes.
...
| `LCG_CHUNK_TEXT_ADVISORY_MAX_CHARS` | No | Advisory character-count threshold for `knowledge_process_chunk`'s `chunk_text` (default `8000`, measured via `chars().count()`, not bytes). Text above this is split internally into threshold-sized units sharing the caller's `chunk_id` — see the Ingestion size contract above. |
