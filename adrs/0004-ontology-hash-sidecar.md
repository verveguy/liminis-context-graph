# ADR-0004: Ontology Hash Sidecar for Drift Detection

**Date**: 2026-05-26
**Status**: Accepted
**Issue**: liminis-graph#98

## Context

When the user edits `ontology.yaml` and restarts liminis-graph, the existing entities and edges in the DB were extracted under the old ontology. The graph has mixed vocabulary. We need to detect this drift and surface it via `knowledge_status` so the UI can recommend a Recreate + re-ingest.
