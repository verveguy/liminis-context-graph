# ADR-0008: Legacy-WAL Translation Layer — Cypher-text/Param-shape vs. Param-value Module Split
...
A new `liminis-graph-core/src/legacy_wal.rs` module houses:
...
- If lbug ever gains a parameterized-query API (see ADR-001), both modules become unnecessary
  and should be removed — the dialect gap is eliminated at source.
- The `regex` crate is now a production dependency of `liminis-graph-core`. It is already
