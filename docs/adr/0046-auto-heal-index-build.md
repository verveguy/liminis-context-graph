...
**Status:** Accepted  
**Date:** 2026-05-24  
**Relates to:** ADR-0042 (reader/writer lock split), issue #58

## Context

ADR-0042 designates `handle_find_entities`, `handle_find_relationships`, and
...
2. **Acquires `write_lock.write()` from within the handler** — a one-time
   exception to the lock-free contract stated in ADR-0042.
...
(per ADR-0043) so that the first post-clear search self-heals again.
...
is the correct mechanism for serialisation and is consistent with ADR-0042's
...
- The lock-free guarantee from ADR-0042 holds for all steady-state searches
