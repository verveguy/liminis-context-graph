# ADR 0043: `ArcSwap<Db>` for Live Database Replacement in `clear_all`
...
1. Acquire the existing `write_lock.write().await` guard (ADR-0042 serialization).
...
`Mutex<Arc<Db>>` serializes every `db.connect()` call across all handlers — including the hot search path. Every read would block on mutex acquisition, conflicting with ADR-0042's explicit goal of keeping search reads off the write serialization path. `ArcSwap` reads are lock-free; only the `store` in `clear_all` has any synchronization cost.
...
- **No change to write serialization**: `clear_all` still acquires the `write_lock` write guard (ADR-0042). Concurrent reads may see a brief gap where the old `Arc<Db>` refers to deleted files if they bypass the lock (hot search handlers). This is a pre-existing architectural gap documented in ADR-0042; `clear_all` cannot close it without adding lock acquisition to the search path.
