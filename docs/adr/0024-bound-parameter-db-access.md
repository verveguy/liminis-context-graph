...
Every DB statement in `lcg-core` was built by **string-interpolating values into
...
- **#128 and #130 are obsoleted** — apostrophes and timestamps are handled by typed binding;
  their escaping/literal logic is gone. ADR-0022 is superseded.
- **ADR-0023 still applies** — `legacy_wal` (`strip_vecf32`, `expand_bulk_property_set`) remains.
