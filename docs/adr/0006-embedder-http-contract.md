# ADR-0006: HTTP Embedding Sidecar Contract
...
liminis-context-graph follows Principle V: no ML runtime in the Rust crate. `HttpEmbedder` in `lcg-core` delegates all embedding work to an external HTTP service, POSTing to `LCG_EMBEDDING_URL` (default `http://127.0.0.1:8765`). This out-of-process boundary introduces a stable API contract between the Rust crate and whatever process serves embeddings.
