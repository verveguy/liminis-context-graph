...
- **Consistent with ADR-0004**: `classify_entities` was added to the `Extractor` trait as a separate method when LLM call semantics differed. Adding `ontology` as a new parameter follows the same "extend the interface when input changes per-call" pattern rather than adding a new method.
