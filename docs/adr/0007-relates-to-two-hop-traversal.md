...
liminis-context-graph operates against workspaces that may have been initialised by either the upstream Python graphiti-core service or by the Rust binary itself. The two runtimes encode RELATES_TO edges differently:
...
- `crates/core/src/db.rs`: `insert_relates_to_edge`, all read methods
- `crates/core/src/schema.rs`: `create_edge_tables`
