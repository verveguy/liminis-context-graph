...
`bench.yml` only. Re-confirmed again during Review (2026-08-18): #398 — the in-flight lbug
upgrade that was expected to add an OpenSSL static-link assertion to this file — is still
open and unmerged, so the audit result stands. This matters beyond `release.yml` itself:
whatever `#398` eventually adds as a load-bearing assertion is only as trustworthy as the
shell it runs under, so that PR should either land after this fix (inheriting nothing extra
needed, since `release.yml` isn't touched here) or itself confirm its assertion actually
fails the step, not just print a warning that a masked exit status would discard.
