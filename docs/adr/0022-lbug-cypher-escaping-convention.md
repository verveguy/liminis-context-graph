...
**Status**: Superseded by ADR-0024 — the string-interpolation/escaping layer this ADR
...
`crates/core/src/replay.rs` reconstructs Cypher statements at WAL replay time by
...
This is identical to the `escape()` function in `crates/core/src/db.rs` (line ~1326),
...
4. **ADR-0015** (`0015-wal-drain-and-flush-pattern.md`): confirms lbug has no parameterized
...
- This ADR becomes moot if lbug exposes a parameterized-query API in a future version.
  At that point, ADR-0015 should be revisited and the `interpolate_params` + `raw_query`
