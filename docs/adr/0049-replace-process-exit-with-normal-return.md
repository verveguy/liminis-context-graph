# ADR-0049: Replace `std::process::exit(0)` with Normal Return in async main

**Status:** Accepted  
**Date:** 2026-05-25  
**Relates to:** ADR-0043 (ArcSwap DB hot-swap), ADR-0047 (auto-heal index build), issue #90
...
- Both `handle_build_indices` and the auto-heal path in `build_indices_once`
  (ADR-0047) are covered by this fix without any change to those handlers.
