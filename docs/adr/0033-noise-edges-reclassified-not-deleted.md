...
**Supersedes**: ADR-0031 (orphaned direct rels concern is now moot — no edges are deleted)
...
2. **ADR-0031 is moot**: The orphaned-direct-rels concern arose from `DETACH DELETE`
   leaving dangling `(Entity)-[:RELATES_TO]->(Entity)` rels. Since no edges are deleted,
   no orphaning occurs and the cleanup query described in ADR-0031 is never needed.
...
- ADR-0031 (superseded) — orphaned direct rels after noise deletion
