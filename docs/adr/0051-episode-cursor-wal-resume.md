# ADR-0051: Episode-Cursor WAL Resume for Checkpoint Recovery

**Status**: Accepted
**Date**: 2026-06-18
**Relates to**: ADR-0046 (degraded-mode startup & in-process recovery), ADR-0047 (auto-heal index build)
**Implemented by**: liminis-graph#151 (engine autonomous recovery); surfaced in the app by liminis#864; progress logging in liminis-graph#152

## Context

ADR-0046 established that a corrupt lbug `db.wal` puts the service into degraded mode, and that `knowledge_recover {strategy:"drop_lbug_wal"}` renames the torn WAL aside and reopens `liminis.db` **at its last lbug checkpoint**. The main DB file is not bricked — lbug simply refuses to auto-truncate a torn WAL tail (a robustness gap versus Postgres/SQLite).
...
2. **No indexes.** A `drop_lbug_wal` reopen does not rebuild FTS/HNSW (see ADR-0047 for why fresh DBs lack indexes), so search is degraded until a replay rebuilds them.
...
1. `drop_lbug_wal` → reopen at the last checkpoint (ADR-0046).
...
- The engine can run this autonomously on startup (liminis-graph#151) so the service self-heals without an external orchestrator — important when liminis-graph runs as a standalone service outside the app.
