...
- **Linux (`x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`)**: both release runner images (`ubuntu-24.04`, `ubuntu-24.04-arm`) ship `libssl-dev` and `pkg-config` preinstalled, which provides `openssl.pc` and the tool to resolve it. No `build-setup.yml` change is functionally required, but `libssl-dev = "*"` and `pkg-config = "*"` are declared explicitly in `[workspace.metadata.dist.dependencies.apt]` (`Cargo.toml`) anyway, so the real dependencies are self-documenting rather than an incidental property of the current runner images.
- **macOS (`aarch64-apple-darwin`)**: the `macos-14` runner has no discoverable OpenSSL by default (only the system `/usr/bin/openssl` 1.1.1 compat shim, which ships no `.pc` file). `.github/build-setup.yml` runs `brew install openssl@3 pkg-config` before the lbug cache-restore steps, scoped with `if: runner.os == 'macOS'` — `pkg-config` is declared explicitly for the same self-documenting reason as Linux's apt deps, even though the runner ships it as `pkgconf`. `HOMEBREW_NO_AUTO_UPDATE=1` prefixes the command so Homebrew doesn't spend several minutes self-updating on every run (per Gemini review feedback on PR #191).
