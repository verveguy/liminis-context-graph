...
- **Name a known-good position** with `knowledge_wal_mark_create {name}` — a lightweight
  ...
  in use by an active mark. `knowledge_wal_mark_list` lists every active mark with its `seq` and
  whether it is currently `reachable` (a cheap min/max check against the WAL content on disk — it
  does not detect a gap in the middle of that range). `knowledge_wal_mark_delete {name}` removes a
