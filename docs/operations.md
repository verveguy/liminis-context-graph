...
(`knowledge_rebuild_from_wal`), which recomputes every vector under the now-running embedder and
updates the recorded identity to match. Until then, `"mismatch"` is a live signal that vector
