...
- **`null`** — unknown position. Only reported when a pre-existing, populated DB has no
  recorded position and the one-time backfill (below) itself fails to find the last episode's
  uuid in the WAL. The documented action is always a full rebuild.
- **`0`** (integer) — a known position: nothing has been applied yet. Reported for a
  fresh/cleared DB (including a pre-existing DB with no `Episodic` nodes at all — the backfill
  treats "no episodes" as "nothing to derive a position from, but known to be empty" and writes
  `0` directly, without a WAL scan), or immediately after `knowledge_clear_all`.
