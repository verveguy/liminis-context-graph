...
- **Rebuild** one group's data from its own WAL directory with `knowledge_rebuild_from_wal
  ...
  - **A referenced group's data outside the directory being replayed is never cleared (issue
    ...
    replayed rows risks a duplicate-key collision replaying the mutating line). The rebuild
    refuses the same way, for a different reason, when the rows this replay would clear for a
    split group currently have a live relationship in the DB pointing into that same group's rows
    that live only in the other, un-replayed stream — clearing just the replayed rows would
    silently sever that relationship, since nothing recreates it. The remedy in either case is the
...
pre-`Db::open()` window as the DB open itself. Autonomous startup recovery ([ADR-0027](adr/0027-autonomous-wal-startup-recovery.md))
...
(`{applied_seq, max_seq, generation, generation_status, hydration_status}`). This is the multi-group view; the flat
`wal.applied_seq`/`wal.max_seq`/`wal.generation`/`wal.generation_status`/`wal.hydration_status` fields described next remain present and
