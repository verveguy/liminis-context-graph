...
| `LCG_EMBEDDING_TIMEOUT_MS` | No | Whole-request timeout, in milliseconds, for every HTTP call made by the embedder transport (`--embedder-http`/`LCG_EMBEDDING_URL`) — single embed, batch embed, and the startup probe (default `30000`, generous enough for a default-sized 64-text batch under normal latency). An unparseable or zero value is rejected with a configuration error rather than silently ignored. Never applies to the UDS transport (`--embedder-uds`). See [ADR-0510](adr/0510-oaiembedder-http-timeouts.md). |
| `LCG_EMBEDDING_CONNECT_TIMEOUT_MS` | No | Connect-phase timeout, in milliseconds, for the HTTP embedder transport, independent of `LCG_EMBEDDING_TIMEOUT_MS` above (default `5000`). Bounds how long a call can stall before completing the TCP/TLS handshake, distinct from the whole-request bound. Same validation and UDS-exemption as `LCG_EMBEDDING_TIMEOUT_MS`. See [ADR-0510](adr/0510-oaiembedder-http-timeouts.md). |
