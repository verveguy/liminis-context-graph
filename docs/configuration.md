...
| `LCG_REPLAY_EMBED_WINDOW_SIZE` | No | Rows buffered before their embedding recompute is resolved via one batched embedder call during WAL replay (default `64`, valid range `1`–`256`). Independent of `LCG_REPLAY_BATCH_SIZE` — this bounds embedder round-trips, not Cypher execution batches. Lower values reduce cancellation latency and peak memory; higher values reduce embedder round-trips on a large rebuild. See [ADR-0486](adr/0486-batch-wal-replay-embedding-recompute.md). |
