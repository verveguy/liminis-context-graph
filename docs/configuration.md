...
| `OPENAI_API_KEY` | No | Convenience fallback for `LCG_EMBEDDING_API_KEY` (see above) — reuses an already-exported OpenAI key. Not a deprecated alias: using it logs no warning. |
...
That's 30 variables as of this page's writing, covering every `env::var`/`lcg_env_var` call site under `crates/*/src`.
