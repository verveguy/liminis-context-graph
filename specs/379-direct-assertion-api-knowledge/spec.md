...
1. **Given** a fresh graph, **When** a test asserts the same sequence of entities and relationships twice in two separate graph instances, **Then** both instances produce identical entity/edge names, attributes, and graph topology (which entity connects to which via which predicate) — modulo timestamps and the entity/edge UUIDs themselves, which are minted per instance and are not required to match. Entity and edge UUIDs are never caller-supplied or derived deterministically (see FR-007); this scenario tests structural determinism, not UUID identity, matching SC-008.
...
- `summary`, on `knowledge_assert_entity`, is optional and defaults to `""` when omitted; a re-assert that omits `summary` fully replaces any previously-set value with `""` (the same full-replace-on-update convention as `attributes`/`labels`), rather than leaving the prior value untouched.
- `fact`, when omitted on `knowledge_assert_relationship`, is auto-generated as `"<source_name> <predicate> <target_name>"`.
- `valid_at`, when supplied on `knowledge_assert_relationship`, accepts both RFC-3339 and lbug's space-delimited read-back timestamp format (e.g. `"2026-06-24 10:00:00"`); an unparseable value fails the call rather than causing a lbug `Binder exception`. `knowledge_assert_entity` does not accept a `valid_at` parameter at all — see FR-022.
- If the configured embedder is unavailable when generating name/fact embeddings, the assertion still succeeds — a same-dimension zero-vector embedding is stored (the embedding columns are fixed-size `FLOAT[N]`; a literal empty vector cannot bind) and a warning is surfaced, matching the existing embedder-unavailability behavior elsewhere in the write path.
...
- **FR-007**: When `knowledge_assert_entity` is called with `entity_uuid`, the system MUST look up that UUID within the given `group_id` and fail the call if no entity with that UUID exists in that group. This is a deliberate narrowing from #179's FR-009, which specified `entity_uuid` as a caller-chosen mint path (a UUID a caller could supply to create a new entity under that exact identifier). `entity_uuid` here is strictly a lookup key with no create fallback — a call can never write an entity into a group at an attacker- or caller-chosen UUID. The consequence is that idempotency for `knowledge_assert_entity` rests entirely on `(name, group_id)` per FR-011, not on `entity_uuid`; a caller cannot pin a UUID across a rebuild or a fresh create.
...
- **FR-012**: `knowledge_assert_entity` MUST generate a name embedding via the configured embedder. If the embedder is unavailable, the call MUST still succeed, storing a same-dimension zero-vector embedding (the `Entity.name_embedding` column is fixed-size `FLOAT[N]`; a literal empty vector cannot bind) and surfacing a warning, rather than failing.
- **FR-026**: `knowledge_assert_entity` MUST accept an optional `summary` string, defaulting to `""` when omitted, and store it on the `Entity` row, consistent with the existing `summary: String` field populated by extraction. A re-assert that omits `summary` MUST fully replace any previously-set value with `""` — the same full-replace-on-update semantics as `attributes`/`labels`, not a partial merge.
...
- **FR-021**: `knowledge_assert_relationship` MUST generate a fact embedding via the configured embedder, with the same embedder-unavailable fallback behavior as FR-012 (a same-dimension zero-vector `fact_embedding` plus a warning, since `RelatesToNode_.fact_embedding` is likewise fixed-size `FLOAT[N]`).
- **FR-022**: `knowledge_assert_relationship` MUST accept an optional `valid_at` timestamp, accepting both RFC-3339 and lbug's space-delimited read-back format, without producing a lbug `Binder exception` on either accepted format. `knowledge_assert_entity` MUST NOT accept a `valid_at` parameter: the `Entity` table has no `valid_at` column (only `RelatesToNode_` carries `valid_at`/`invalid_at`/`expired_at`), and mapping it onto `created_at` (which means something different and which merge forwarding already mutates) or burying it in `attributes` (where nothing reads it back) were both explicitly rejected in favor of simply not exposing the parameter on this tool.
