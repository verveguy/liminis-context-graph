...
- **`recovery.rs`'s `derive_episode_cursor`** (used by `knowledge_recover`'s `drop_lbug_wal` strategy and by the binary's own startup auto-heal) — investigated during Implement following corroborating CI evidence (a flaky timeout in `mcp_real_corpus_admin_lifecycle_e2e`, PR #452, run [32483319399](https://github.com/verveguy/liminis-context-graph/actions/runs/32483319399), job 96776221580). Confirmed to be a **sibling defect sharing this issue's exact root cause**, not a false alarm: `derive_episode_cursor(conn, group_id, wal_dir)` looks up the latest episode via `conn.get_latest_episode_uuid(group_id)`, scoped to the request's own `group_id` (the WAL directory's owning group). For the migrated real-corpus fixture, every episode is actually tagged `group_id: "apollo_program"`, not `"liminis"`, so the lookup always finds nothing, always concludes `cursor_reason: "no_episodes"`, and always forces `from_seq: 0` — triggering a full, unnecessary WAL replay against a database that (post checkpoint-drop) already holds the fixture's complete data, producing an identical flood of duplicate-primary-key `[WAL WARN]` lines to this issue's own mechanism, via a different function and trigger. This was not scoped by Research or Plan, and fixing it needs its own design pass (recovery's cursor derivation has no obvious analogue to the guard's "scan content for every referenced group_id" fix — a resume cursor is inherently a single seq number, not a per-group clear decision). Left for a dedicated follow-up issue rather than folded into this one's already-approved Plan.
...
- `crates/core/src/recovery.rs` — `derive_episode_cursor` (see Out of Scope above: a sibling defect discovered during Implement, sharing this issue's root cause but not fixed by it).
