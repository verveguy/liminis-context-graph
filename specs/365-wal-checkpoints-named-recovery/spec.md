...
4. **Given** a fresh workspace with an empty WAL and a fresh database (`applied_seq == 0`, a *known* position per ADR-0353, not `null`/unknown), **When** `knowledge_wal_mark_create {name: "fresh"}` is called, **Then** it succeeds and records a checkpoint at `seq: 0` — but that seq is inherently ambiguous: `applied_seq == 0` also results once the WAL's very first line (`seq: 0`, since `WalWriter`'s `global_seq` counter starts at `0`) has been committed, so this checkpoint does not by itself distinguish "nothing applied" from "WAL line 0 applied." Restoring to it (`to_seq: 0`, inclusive) will include WAL line 0 whenever one exists at restore time. See FR-005.
...
- **`applied_seq == 0` on create**: accepted (it is a known position, not `null`), but ambiguous between "nothing applied" and "WAL line 0 applied" — an ambiguity inherited from the existing `applied_seq` contract (ADR-0353), not resolved by this feature (Story 1, Scenario 4; FR-005).
...
- **FR-005**: `applied_seq == 0` MUST be accepted by `create` as a known position (FR-004 only rejects `null`) — but `0` is inherently ambiguous under the existing `applied_seq` contract (ADR-0353): it results both from a genuinely fresh/cleared database with nothing applied, and from a database that has committed exactly the WAL's first line (`seq: 0`, since `WalWriter`'s `global_seq` counter starts at `0`, `wal.rs:101`). A checkpoint recorded at `seq: 0` therefore does not, by itself, distinguish "nothing applied" from "WAL line 0 applied," and restoring to it (`to_seq: 0`, inclusive per FR-014) will include WAL line 0 whenever one exists. This ambiguity is inherited from the existing `applied_seq` contract, not introduced by this feature, and this spec does not resolve it. Candidate resolutions — rejecting `create` in the genuinely-empty case, carrying an explicit empty/`None` distinction in the checkpoint record analogous to `to_seq`'s `None`/`Some(0)` discipline (#362), or documenting that `seq: 0` checkpoints are not meaningful and discouraging their creation — are left to the Research/Plan stages; whichever is chosen MUST be reflected in `knowledge_wal_mark_create`'s documented behavior.
...
- **FR-014**: No new restore/rebuild method is introduced. Recovery to a checkpoint MUST be performed entirely through the existing bounded-replay primitive: `knowledge_rebuild_from_wal {from_seq: 0, to_seq: <checkpoint seq>, force_clear: true}` (inclusive of `to_seq`, per #362).

### Key Entities *(if the feature involves data)*

- **Checkpoint (WAL mark)**: a named, retained WAL position — a `{name, seq}` pair recorded in the checkpoint store. `name` is unique among currently active (non-deleted) checkpoints; a name may be reused after its prior checkpoint is deleted. `seq` is the WAL sequence number the checkpoint refers to, captured from the attached database's `applied_seq` at creation time (FR-003); a `seq` of `0` carries the ambiguity described in FR-005.
...
- Fully resolving the `applied_seq == 0` ambiguity described in FR-005 — this spec flags it and constrains the acceptable resolutions but leaves the choice to Research/Plan.
