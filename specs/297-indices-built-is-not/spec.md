...
- For the reopen-based strategies (`drop_lbug_wal`, `restore_from_backup`), a `knowledge_recover` call that fails outright (DB doesn't reopen, no backup file found, etc.) must leave `indices_built` unchanged at its prior value — these strategies never invalidate indices in the first place, so a failed call does not make the *previous* state's indices any less valid or any more valid. This is distinct from the rebuild-based strategies' failure behavior in FR-002, which invalidate indices as part of the attempt and so must land on `false`, not the pre-attempt value, when the rebuild fails.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: A successful runtime recovery — via any `knowledge_recover` strategy (`drop_lbug_wal`, `rebuild_from_workspace_wal`, `restore_from_backup`) or via `knowledge_recover_full` — MUST result in `AppState.indices_built` being `true` once the recovery reports success and the resulting database is trusted to be currently-indexed (rebuilt indices for the rebuild-based strategies; per the Assumptions section's trust model, not independently re-verified, for the reopen-based ones).
