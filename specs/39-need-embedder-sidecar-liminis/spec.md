...
liminis-graph follows Principle V: no ML runtime in the Rust crate. Embeddings are delegated to an external HTTP service via `HttpEmbedder`, which POSTs to `http://127.0.0.1:8765` by default (env var `LCG_EMBEDDING_URL`). The upstream Python graphiti-core service handled this transparently by loading `sentence-transformers` + `bge-base-en-v1.5` in-process.
...
2. **Given** the sidecar is running, **When** a liminis-graph `HttpEmbedder` calls it with any non-empty text, **Then** the returned vector has exactly 768 elements (matching `LCG_EMBEDDING_DIM` default) and all elements are finite `f32`-representable floats.
...
- `LCG_EMBEDDING_URL` points to a different host:port → sidecar binds to the configured address, not hardcoded `127.0.0.1:8765`.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The sidecar MUST expose `POST /` (path configurable via `LCG_EMBEDDING_PATH`, default `/`) accepting `{"text": "<string>", "model": "<string>"}` and returning `{"embedding": [<floats>]}` with HTTP 200.
...
- **FR-003**: The sidecar MUST bind to the address and port derived from `LCG_EMBEDDING_URL` (default `http://127.0.0.1:8765`). If the env var is unset, the defaults apply.
- **FR-004**: The sidecar MUST load the model named by `LCG_EMBEDDING_MODEL` (default `bge-base-en-v1.5`) via `sentence-transformers` at startup. The `/health` endpoint MUST return 503 until the model is fully loaded.
...
- **FR-010**: The liminis-graph README MUST be updated to document that `HttpEmbedder` requires a running embedder service at `LCG_EMBEDDING_URL`, name the sidecar script, and explain how to start it manually.
...
- **`LCG_EMBEDDING_URL`**: Env var controlling where `HttpEmbedder` sends requests and where the sidecar binds. Both sides read the same variable.
- **`LCG_EMBEDDING_MODEL`**: Env var controlling which `sentence-transformers` model the sidecar loads. `HttpEmbedder` sends this value in the `model` field of every request.
- **`LCG_EMBEDDING_DIM`**: Env var controlling the expected embedding dimension. The sidecar's output vector length MUST match this value (default 768).

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: `POST /` with a non-empty text string returns a 768-element (or `LCG_EMBEDDING_DIM`-element) float array within 5 seconds on a warm sidecar (model already loaded).
...
- **SC-005**: The liminis-graph README contains a section explaining the sidecar dependency, the `LCG_EMBEDDING_URL` env var, and how to start the sidecar manually.
...
- The `sentence-transformers` library can load `bge-base-en-v1.5` and produce 768-dimensional vectors matching what the upstream Python graphiti-core service previously produced. The embeddings do not need to be byte-identical to prior runs; semantic search quality within existing-graph tolerance is sufficient.
...
- The `model` field in `POST /` requests from `HttpEmbedder` will always match `LCG_EMBEDDING_MODEL`. A mismatch is treated as a client error (HTTP 400) rather than triggering a model swap.
...
- liminis-app lifecycle wiring (`backend = rust` process management) — covered by a separate follow-up issue: "Wire embedder sidecar into liminis-app context-graph lifecycle for `backend = rust`".
