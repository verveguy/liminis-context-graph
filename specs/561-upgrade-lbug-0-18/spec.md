...
**Retarget update (2026-09-15, later): 0.20.4 → 0.20.3.** Windows testing found that lbug 0.20.4's core crashes natively in the only published Windows `vector`/`fts` extensions (`LadybugDB/ladybug#971`). Upstream publishes extensions per minor version, and the `v0.20.0/win_amd64` files are 0.20.2-era builds (PE link time and highest embedded version string both say so), ABI-compatible with the 0.20.2 and 0.20.3 cores but not 0.20.4's. On 0.20.3 the same bytes ran vector 110/110 and fts 25/25 clean across two OpenSSL DLL vendors; on 0.20.4 fts failed every run and vector intermittently. 0.20.3 keeps storage version 47, `LBUG_EXTENSION_VERSION` `0.20.0`, and the `ladybug#883` fix, so every goal below is unchanged. **Wherever this spec's requirements and success criteria say 0.20.4 as the pin (FR-001, FR-004, FR-013, FR-014, SC-001, SC-002, SC-009 and the user stories that restate them), read 0.20.3.** The 0.20.2 → 0.20.4 history below is kept as the record of how the target moved.

**Retarget update (2026-09-15): 0.20.2 → 0.20.4.** After the 0.20.2 evaluation above, upstream closed `ladybug#883` (see below) as fixed in `0.20.3`, and `0.20.4` shipped shortly after — both newer than the `0.20.2` this issue originally targeted. This issue's target pin is now **0.20.4**, not 0.20.2: it carries every fix 0.20.2 did (`#877`/`#878`, the forgone-fix set, the deadlock-free 0.20.x behavior) plus the `#883` fix itself. The extension-directory trap below is unchanged — `LBUG_EXTENSION_VERSION` must still resolve to `0.20.0` regardless of which 0.20.x patch is pinned, re-verified directly against the 0.20.4 crate's `lbug-src/CMakeLists.txt` and the live CDN for all four supported platforms (`osx_arm64`, `linux_amd64`, `linux_arm64`, `win_amd64`). Separately, an intermittent SIGSEGV was observed in CI's `backfill_summary_embeddings_wal` test binary on the *current*, still-shipping 0.18.1 pin, with no assertion failure; whether 0.18.1 already carries a variant of `ladybug#883`'s defect is an open question nobody has tested directly (upstream's repro only covered 0.20.x) — if it does, that strengthens the case for shipping this upgrade sooner, but it is not a blocker for this issue's own acceptance criteria.

Separately, 0.20.2 introduced `enable_cached_prepared_statement` (`src/include/main/settings.h:170`), which 0.20.1 lacked, giving operators a way to disable the cached-plan path entirely if `ladybug#883` were ever hit in production. `ladybug#883` has since been closed upstream as fixed in `0.20.3`, and this issue's target (`0.20.4`) postdates that fix — so the setting is no longer a live mitigation for the version this issue actually ships, but it remains present and is confirmed settable as defense-in-depth. This issue confirms the setting exists and is wired up for use if needed; it does not enable it by default (see Out of Scope).
...
As a maintainer of this repository's dependency footprint, I want the `lbug` crate pin (`Cargo.toml`) and the native prebuilt-bundle version pin (`.cargo/config.toml`'s `LBUG_VERSION`) bumped together from `0.18.1` to `0.20.4` in the same commit, with any resulting API differences absorbed, so that the crate pin and the native bundle never skew apart and the workspace compiles and passes its full test surface.
...
**Independent Test**: Bump `lbug = "=0.20.4"` in `Cargo.toml` and `LBUG_VERSION = "0.20.4"` in `.cargo/config.toml` in the same commit, run a local build, and run the full CI suite including all five real-corpus e2e jobs.

**Acceptance Scenarios**:

1. **Given** `lbug = "=0.18.1"` in `Cargo.toml` and `LBUG_VERSION = "0.18.1"` in `.cargo/config.toml`, **When** both are bumped to `0.20.4`, **Then** both changes land in the same commit.
...
3. **Given** the bumped pins, **When** the workspace is built, **Then** it compiles cleanly, with any lbug Rust/C API differences between 0.18.1 and 0.20.4 absorbed at call sites.
...
**Independent Test**: Set `LBUG_EXTENSION_VERSION` to `0.20.0` (not `0.20.4`) and `LBUG_CRATE_VERSION_VERIFIED_AGAINST` to `0.20.4`, confirm `extension_version_was_verified_against_current_lbug_pin` passes, confirm `scripts/stage-lbug-extensions.sh` succeeds against the live CDN for the new version, and evaluate (adding coverage if needed) whether existing tests would catch extension bytes staged under a directory name that doesn't match what resolution expects.

**Acceptance Scenarios**:

1. **Given** the lbug crate pin moves to `0.20.4`, **When** `LBUG_EXTENSION_VERSION` (repo root) is updated, **Then** it is set to `0.20.0`, verified empirically against the CDN (`https://extension.ladybugdb.com/v0.20.0/...` serves both `vector` and `fts`; `v0.20.1` through `v0.20.4` all 404) — not to `0.20.4`.
2. **Given** the same pin move, **When** `LBUG_CRATE_VERSION_VERIFIED_AGAINST` is updated to `"0.20.4"`, **Then** the existing `extension_version_was_verified_against_current_lbug_pin` test passes.
...
**Independent Test**: Obtain or generate a database at storage version 42 (e.g. by creating one under the currently-pinned 0.18.1 binary before the pins are bumped), open it under the new 0.20.4-based binary, and confirm it migrates and serves correct reads. The existing `storage_v41_migration` test (from #529) continues to pass unmodified, demonstrating the older migration path is not broken by this change.

**Acceptance Scenarios**:

1. **Given** a pre-existing database at storage version 42, **When** it is opened under the 0.20.4-based binary, **Then** it opens successfully and migrates to storage version 47 without any manual operator step.
...
As a maintainer aware that `ladybug#883` (a SIGSEGV in the cached-prepared-statement path) was open and unfixed upstream at the time this story was first written, I want a test or manual run that issues a substantially larger number of parameterized queries in one session than the existing suite does, so that this upgrade has direct evidence about long-session stability rather than relying solely on the short suite's clean pass.

**Why this priority**: `#883` was explicitly not fixed by 0.20.2 per the issue's original framing; that framing has since changed — `#883` was closed upstream as fixed in `0.20.3`, and this issue's actual pin (`0.20.4`) postdates the fix — so this story now confirms the fix holds under sustained load rather than merely characterizing an unfixed risk. It remains P2 relative to User Stories 1–5, which are prerequisites for shipping at all.
...
2. **Given** that `ladybug#883` has been fixed upstream in `0.20.3` (which this issue's `0.20.4` pin postdates), **When** this story's run passes, **Then** the PR description states plainly that the fix is confirmed under sustained load, and notes `enable_cached_prepared_statement` (see Background) remains available as defense-in-depth should a future regression in this path occur.
...
1. **Given** the completed upgrade, **When** `CHANGELOG.md` is inspected, **Then** its `[Unreleased]` section records the lbug 0.18.1 -> 0.20.4 bump and the 42 -> 47 storage migration in one entry, including the rollback procedure (stop the service, move `.lcg/db/` aside, start the old binary, it rebuilds from the WAL).
...
- What happens if a storage-v42 database fails to open or migrate cleanly under 0.20.4? This blocks the issue — User Story 4 requires an explicit, automatic, no-manual-step migration, not a documented workaround.
...
- What happens if the long-session run (User Story 6) surfaces a crash despite `ladybug#883` being closed as fixed upstream in `0.20.3`? This blocks the issue — a crash under a version that postdates the upstream fix would mean the fix doesn't hold for this repo's workload and needs to be escalated upstream (or mitigated via `enable_cached_prepared_statement`) before this bump ships, not merely documented as residual risk.
- What happens if the 0.20.1 deadlock (the reason for the original rollback) reappears under 0.20.4 despite the retest evidence in Background? This blocks the issue; the retest evidence is preliminary confirmation, not a substitute for the full CI suite (User Story 1) and long-session run (User Story 6) passing on this branch.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: `Cargo.toml`'s `lbug` pin and `.cargo/config.toml`'s `LBUG_VERSION` pin MUST both move from `0.18.1` to `0.20.4`, changed in the same commit.
...
- **FR-003**: The repo-root `LBUG_EXTENSION_VERSION` file MUST be updated to `0.20.0` (not `0.20.4`), matching the extension-directory version the lbug 0.20.4 crate actually compiles against and the CDN actually publishes under (verified empirically: `v0.20.0` serves both extensions on all four supported platforms; `v0.20.1` through `v0.20.4` all 404).
- **FR-004**: `LBUG_CRATE_VERSION_VERIFIED_AGAINST` (`crates/core/src/lbug_extension_home.rs`) MUST be updated to `"0.20.4"` in the same change as FR-003, so the existing `extension_version_was_verified_against_current_lbug_pin` tripwire test passes and continues to guard future bumps.
...
- **FR-009**: A test or documented manual run MUST exercise at least 1,000 parameterized query executions in a single session and confirm no crash, hang, or stale result — evidence for long-session behavior beyond what the existing short suite reaches, confirming the `ladybug#883` fix (closed upstream in `0.20.3`, included in this issue's `0.20.4` pin) holds under sustained load.
...
- **FR-012**: Version-specific comments in `Cargo.toml` and `.cargo/config.toml` that describe 0.18.1-specific facts (e.g. OpenSSL linkage behavior, pkg-config discovery notes) MUST be reviewed against 0.20.4 and corrected wherever the underlying fact has changed; facts that still hold MUST be left as-is.
- **FR-013**: Any lbug Rust/C API differences between 0.18.1 and 0.20.4 MUST be absorbed so the workspace compiles cleanly; compiling cleanly alone is not sufficient acceptance evidence — full CI, including all five real-corpus e2e jobs, MUST also pass.
- **FR-014**: `CHANGELOG.md`'s `[Unreleased]` section MUST record the lbug 0.18.1 -> 0.20.4 bump and the single 42 -> 47 storage migration, including the rollback procedure (stop the service, move `.lcg/db/` aside, start the old binary, it rebuilds from the WAL). If a `docs/releases/<version>.md` file is created for the release carrying this change before the PR merges, it MUST be updated consistently; none exists yet, so this is conditional, not a hard gate today.
...
- **SC-001**: `Cargo.toml` pins `lbug = "=0.20.4"` and `.cargo/config.toml` pins `LBUG_VERSION = "0.20.4"`, both changed in the same commit.
- **SC-002**: `LBUG_EXTENSION_VERSION` is `0.20.0` and `LBUG_CRATE_VERSION_VERIFIED_AGAINST` is `"0.20.4"`; `extension_version_was_verified_against_current_lbug_pin` passes; `scripts/stage-lbug-extensions.sh` succeeds for every supported platform against the live CDN.
...
- **SC-009**: `CHANGELOG.md` describes the lbug 0.18.1 -> 0.20.4 bump, the 42 -> 47 migration, and the rollback procedure.
...
- The `LBUG_EXTENSION_VERSION` value (`0.20.0`) and the CDN-availability probe (`v0.20.0` serves both extensions on all four supported platforms; `v0.20.1` through `v0.20.4` all 404) are treated as an established fact from manual verification already performed — re-confirmed directly against the 0.20.4 crate's `lbug-src/CMakeLists.txt` and the live CDN as part of the retarget from 0.20.2 to 0.20.4 — not something a future bump needs to redo from scratch, though it should still be re-confirmed at the time of any subsequent bump, since CDN state can change.
...
- lbug's public Rust/C API surface used by this repo (`crates/core`, `crates/service`) has no breaking changes between 0.18.1 and 0.20.4 beyond what can be resolved mechanically at call sites; if a resolution is not mechanical, FR-013 still governs the outcome (compile-clean and full CI green), and the resolution approach is a Research/Plan-stage decision, not a spec change.
- 1,000 parameterized query executions (FR-009) is a concrete, testable floor derived from the issue's own qualitative framing ("hundreds of parameterized queries in one session"); this floor remains meaningful evidence even though `ladybug#883` is now fixed in the pinned `0.20.4` — it demonstrates the fix holds under load rather than merely that the bug doesn't reproduce in a short run. Research/Plan MAY increase this figure, but MUST NOT reduce it below this floor without recording why in the Plan stage.
...
- Enabling `enable_cached_prepared_statement` by default. This issue confirms the setting exists and is available as a documented defense-in-depth lever; actually flipping it (which would disable the cached-plan path's performance benefit entirely) is a separate decision to be made only if a future regression in this path is actually hit, and would warrant its own follow-up issue.
...
- Root-causing `ladybug#883` itself — moot for this issue, since it was closed upstream as fixed in `0.20.3`, which this issue's `0.20.4` pin already includes. User Story 6's long-session test now serves as regression coverage confirming the fix holds under this repo's workload, not risk characterization of an open bug.
...
- Upstream `LadybugDB/ladybug` issues/PRs `#877`, `#878`, `#845`, `#837`, `#864`, `#894`, `#884`, `#883`, `#911` (this repo's upstream deadlock report), and releases 0.19.1 through 0.20.4. `ladybug#883` was closed upstream as fixed in `0.20.3`.
- `src/include/main/settings.h:170` (upstream `LadybugDB/ladybug`) — `enable_cached_prepared_statement`
- `lbug-src/CMakeLists.txt:525` (upstream `LadybugDB/ladybug`, 0.20.4 crate) — compiles in `LBUG_EXTENSION_VERSION="0.20.0"`, the empirical basis for FR-003 (re-verified directly against the 0.20.4 crate after the retarget from 0.20.2)
