...
- **Hosted vs. self-hosted ceiling (FR-006)**: the proportional formula (FR-002) is uniform across providers — it encodes what the extraction task needs, which doesn't vary by who serves the model. The absolute ceiling is lower on the hosted Anthropic path, reflecting that a hosted runaway costs real metered spend while a local runaway costs GPU time; hosted has also empirically needed less headroom (Haiku measured 0–0.4% errors and never approached the cap, while qwen emits 15–32% more content for the same input). The per-call ceiling is a correctness/runaway backstop, not a substitute for a per-run cost-control mechanism (see Out of Scope).
...
- **FR-005**: The decision MUST be recorded in an ADR numbered with this issue number (`docs/adr/0307-*.md`) per this repository's ADR convention. The ADR MUST record the reasoning behind both decisions, including specifically: for FR-004, the measurement-corruption argument (silent `Ok(vec![])` misattributes suppressed edges to the model's own recall, as demonstrated by the qwen3.6-35b-a3b comparison); for FR-006, that the per-call ceiling is a correctness/runaway backstop rather than the cost-control mechanism.
- **FR-006**: The token-budget policy MUST use the same proportional formula (FR-002) on both the hosted Anthropic path and self-hosted/OAI-compatible models, but MUST apply a lower absolute ceiling on the hosted path, reflecting that a hosted runaway costs real metered spend while a local runaway costs GPU time (see Background: Resolution).
...
- The exact proportionality formula, floor, and ceiling values for FR-002, and the exact magnitude of the hosted-path ceiling relative to the self-hosted ceiling (FR-006), are empirical/technical determinations left to Research and Plan, bounded by the testable outcome in SC-003 — this spec establishes that a differential MUST exist and MUST share a formula, not the specific numbers.
...
- Determining the exact proportionality constant, floor, and ceiling values for the token-budget formula, or the exact hosted-vs-local ceiling differential — an empirical Research/Plan task, not a spec-level decision (see Assumptions).
...
- A per-run or aggregate cost-budget guard across all chunks in a run. FR-006's ceiling is a per-call correctness/runaway backstop; if metered hosted spend needs a harder guarantee than a lower per-call ceiling provides, a separate aggregate budget mechanism is a follow-up, not part of this issue.
