...
A user runs the liminis-context-graph engine on macOS with the CoreML sidecar running and does **not** set `ANTHROPIC_API_KEY`. They pass `--extractor-uds` pointing at the sidecar's socket (or `LCG_EXTRACTION_URL`/`--extractor-http` at any other OpenAI-compatible endpoint they've chosen) and call `knowledge_process_chunk` (or `knowledge_add_episode`) with ordinary text. Today this call fails with a 401 from Anthropic's API. After this change, the call succeeds, using the selected local endpoint's `/v1/chat/completions` route for extraction, and returns typed entities and relationships just as the Anthropic path would.
...
**Independent Test**: Start the sidecar, unset `ANTHROPIC_API_KEY`, start the engine with `--extractor-uds` pointing at the sidecar's socket, and call `knowledge_process_chunk` with a short passage containing at least one clear entity and relationship (e.g., "Alice works at Acme Corp"). Assert the call succeeds and the response contains at least one entity and one edge.

**Acceptance Scenarios**:

1. **Given** the sidecar is running, `ANTHROPIC_API_KEY` is unset, and `--extractor-uds`/`--extractor-http` explicitly selects a local endpoint, **When** `knowledge_process_chunk` is called with text describing an entity and a relationship, **Then** the call succeeds and returns typed entities and edges (not a 401 or connection error).
2. **Given** the sidecar is running and `ANTHROPIC_API_KEY` is unset, **When** the engine starts with no `--extractor-uds`/`--extractor-http` flag and no `LCG_EXTRACTION_URL` set, **Then** the engine does **not** auto-detect the sidecar's default socket for extraction (unlike the embedder) — it fails to start with the FR-011 actionable error, per the design decision recorded in the Assumptions section and ADR-0038.
...
- **No provider reachable at all**: neither `ANTHROPIC_API_KEY` nor an explicitly-configured local endpoint (`--extractor-uds`/`--extractor-http`/`LCG_EXTRACTION_URL`) is available. The system must surface one clear, actionable error identifying that no extraction provider is configured — not a raw connection-refused error or an unhandled panic. Notably, a running sidecar socket alone does **not** count as "reachable" for this purpose — see FR-006, tier 3.
...
- **FR-006**: Endpoint selection precedence, from highest to lowest, MUST be:
  ...
  3. No explicit local flag and no `ANTHROPIC_API_KEY` — consult `LCG_EXTRACTION_URL` (an explicit env-var override), else fail with the FR-011 error. Deliberately unlike the embedder, tier 3 does **not** auto-detect the default UDS socket by mere existence: prior evaluation (see Assumptions) found the bundled sidecar's extraction backend inadequate for extraction quality, so a running sidecar alone must never silently become the extraction provider — reaching it still requires an explicit `--extractor-uds`/`--extractor-http` flag or `LCG_EXTRACTION_URL`.
- **FR-007**: A reachable local endpoint MUST NOT override or bypass a configured `ANTHROPIC_API_KEY` when no explicit local-endpoint flag is given (see FR-006, tier 2) — this is the backward-compatibility guarantee for existing users.
...
- **SC-001**: With the local sidecar running, `ANTHROPIC_API_KEY` unset, and `--extractor-uds`/`--extractor-http` explicitly selecting a local endpoint, `knowledge_process_chunk` on a chunk describing at least one entity and relationship succeeds and returns at least one typed entity and one typed edge.
...
- **Prior-art constraint (recorded during Validate, PR #223 review)**: private-repo evaluation predating this issue — ported into this repo by #227 (documentation) and #228 (in-repo eval harness) — assessed Apple Foundation Models specifically for entity/relationship extraction and found it inadequate (insufficient context window and capability for the task's quality bar), with a standing instruction not to wire the local-inference socket into extraction until a fresh capability pass. That same evaluation identified `qwen3.6-27b`, served via any OpenAI-compatible endpoint (e.g. `mlx_lm.server`), as a local model that does meet the quality bar. This is why FR-006 tier 3 has no default-UDS-socket auto-detection (unlike the embedder, whose analogous tier 3 does auto-detect): the mechanism this issue builds (`OaiExtractor`, the CLI flags, the precedence ladder) is sound and reused as-is, but the engine must never make the specific choice of "bundled sidecar for extraction" on the operator's behalf. See ADR-0038's Decision 3 and Consequences for the full rationale.
