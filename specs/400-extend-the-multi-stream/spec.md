...
A fourth phase was added to this issue's scope, motivated by #414: `knowledge_status.wal_groups[*].generation` was observed as `null` across a full two-channel production hydrate, making ADR-0387's reset detection inert. The root cause was external to lcg — the producer's publish step copied `*.jsonl` only, and `.wal-generation.json` is a dotfile a shell glob does not match by default; lcg itself mints and reports generation correctly for streams it creates. The lesson for this issue is that **a stream can arrive with no generation sidecar at all, and nothing in the current suite says so**: every phase specced here builds its streams through lcg's own API, so every stream in the test has a generation by construction — the arrived-without-one case was unreachable in the suite as written, and is exactly the condition that let #414 reach production undetected.
...
**Why this priority**: This is one of four named compositions this issue exists to protect, each guarding a defect class the existing nine-phase test cannot reach because it never republishes a stream under a new generation.
...
### User Story 4 - A stream that arrives with no generation sidecar is detected, not silently inferred (Priority: P1)

A WAL stream can arrive with `*.jsonl` content but no `.wal-generation.json` — not because it was never hydrated, but because whatever copied/published it dropped the sidecar (#414's confirmed root cause: a `*.jsonl`-only glob silently drops the dot-namespace while appearing to publish the complete stream). This is a different condition from User Story 1's reset: Story 1 exercises a *known-to-known* generation mismatch; this story exercises *generation unknown entirely* — a distinct state per ADR-0387, and one this suite has never produced, since every stream it builds is created directly through lcg's own API and therefore always has a generation. #414 found this exact gap reachable in production, undetected until someone happened to read a status field.

**Why this priority**: This is the phase that would have caught #414 on day one — the fixture already stands up three groups, so it is a file deletion plus a status assertion, and it guards the exact condition that reached production with zero suite coverage.

**Independent Test**: Remove `.wal-generation.json` from a group's WAL directory that already has `*.jsonl` content (simulating a publish step that dropped the dot-namespace), then call `knowledge_status` and hydrate/replay that group; confirm the unknown-generation condition is surfaced rather than silently treated as an ordinary replay.

**Acceptance Scenarios**:

1. **Given** a group's WAL directory has `*.jsonl` content but its `.wal-generation.json` sidecar has been removed (content exists; the sidecar does not), **When** `knowledge_status` is called for that group, **Then** it reports the condition for that group, distinguishable from both "known generation" and "no stream yet."
2. **Given** the same missing-sidecar condition, **When** that group is hydrated/replayed, **Then** whatever behavior #414's R3 guard settles on — warn-and-continue or refuse-to-advance — is exercised end to end, rather than silently degrading to a seq-only inference. If #414 lands before this phase is implemented, this phase MUST assert #414's chosen semantics directly rather than restating them independently.
3. **Given** the missing-sidecar condition is introduced for one group, **When** it's introduced, **Then** the other groups sharing the same WAL root are unaffected by it.
...
- What happens to a WAL stream for a group that is party to none of the new phases? Same standard this test already applies to existing phases (e.g. B during A's purge/restore): it must be asserted unchanged, on disk, not only via `knowledge_status`.
...
- **FR-006**: The test MUST gain a phase covering a stream that arrives with no generation sidecar (#414) — the shape a mis-published stream has in practice. Construct it by removing `.wal-generation.json` from a group's WAL directory before hydrating/replaying it, rather than by any lcg API call, since no API produces this state. It MUST assert that (i) `knowledge_status` reports the condition for that group, (ii) whatever behavior #414's R3 guard settles on — warn-and-continue or refuse-to-advance — is exercised end to end, rather than silently degrading to a seq-only inference, and (iii) other groups sharing the same WAL root are unaffected. If #414 lands before this phase is implemented, this phase MUST assert #414's chosen semantics rather than restating them independently.
...
- **Generation status**: A per-group classification — no stream yet / known generation / unknown (missing or corrupt `.wal-generation.json`) — that `knowledge_status` and `knowledge_rebuild_from_wal` must make distinguishable rather than collapsing "no stream yet" and "unknown" to the same `null` value; the subject of User Story 4 (#414).
...
- **SC-004**: The missing-generation-sidecar phase (FR-006) fails if `knowledge_status` or `knowledge_rebuild_from_wal` ever silently treats an unknown generation as an ordinary, unremarkable replay with no observable trace — independent of which specific guard behavior (warn-and-continue vs. refuse-to-advance) #414 ultimately settles on.

## Assumptions

- `crates/service/tests/mcp_multistream_e2e.rs`, as merged by PR #395, already exists with the nine phases and ten assertions described in this issue's Background, and already provides the reusable fixture (three-group setup, `McpClient`, `wal_snapshot`, `c_bindings`/`binding_states_into` helpers) this issue's new phases build on (confirmed present in the codebase as of this spec).
- `crates/core/tests/wal_generation_reset.rs`, `merge_entities.rs`, and `cross_group_pointers.rs` already provide first coverage of generation reset, cross-group merge, and ambiguous binding respectively at the `crates/core` integration level against a real database (confirmed present as of this spec). This issue adds composition-level coverage at the live-process/MCP boundary for these three; it is not the first test of any of them.
- ADR-0369 (resolvable cross-group pointers, the `bound`/`unbound`/`ambiguous` tri-state), ADR-0371 (a merge in group G touches only G's data; `merged_into` forwarding), and ADR-0387 (WAL stream generation identity and the `knowledge_rebuild_from_wal` self-heal path) are all Accepted and already implemented — the mechanisms User Stories 1-3 exercise already exist in the codebase, and this issue adds test coverage for them rather than new product behavior. User Story 4/FR-006 is the exception: the "unknown generation" guard it exercises is #414's own deliverable and had not landed as of this spec (see below) — this issue still adds no product code of its own, but that specific phase's expected behavior is contingent on #414 rather than on already-shipped code.
...
- User Story 1 (FR-001) and User Story 4 (FR-006) cover opposite failure modes of the same generation-identity mechanism and neither substitutes for the other: an lcg-built stream always has *some* generation, so a republish-under-new-identity test (Story 1) exercises the happy path of that mechanism (a known-to-known mismatch); it would not have caught #414, whose defect was a stream arriving with *no* generation at all (Story 4).
- #414 ("WAL stream `generation` is null end-to-end — ADR-0387 reset detection is inert") is open and in Draft status as of this round, with its own unresolved question (Q4: whether its R3 guard warns-and-proceeds or refuses-to-advance on an unknown generation). FR-006/User Story 4 does not depend on that question being resolved in order to be written — the phase can be constructed today by removing `.wal-generation.json` from a hydrated group's directory regardless of #414's implementation status — only its Acceptance Scenario 2 assertion needs #414's chosen semantics to be locked in before its exact expected outcome can be finalized.

## Out of Scope

- Fixing #392 (the `rebind_pointers` staleness-gate bug) — unrelated to and unaffected by this issue's new phases, which are ordered specifically to avoid depending on it (see Edge Cases).
- Deciding #414's own warn-and-continue-vs-refuse-to-advance semantics (its Open Question Q4) — that decision belongs to #414 itself; User Story 4/FR-006 asserts whichever behavior #414 settles on (see Assumptions).
- Any product-facing behavior change, new MCP method, or schema change — this issue is purely test coverage for behavior that already exists (or, for FR-006, that #414 will implement) and is already covered at the `crates/core` level or specified separately.
- Adding compositions beyond the four named (generation reset, cross-group merge, ambiguous resolution, missing generation sidecar) — a further composition gap, if one is found, is a follow-up issue.
- Deciding the exact insertion point/phase numbering for the new phases within the test file (beyond the merge-phase-before-purge ordering constraint this spec fixes), the internal mechanism used to simulate an externally-republished WAL generation or a missing generation sidecar, and any refactoring of the existing nine-phase test's structure — implementation detail left to the Research/Plan/Implement stages.
...
- Motivating issues: #394/PR #395 (the original port), #392 (the third defect this test surfaced; closed, milestone 0.13.1; its fix had not yet merged to `main` as of this spec's base commit — see Assumptions), #368/#371 (cross-group merge), #369 (resolvable pointers), #387 (generation identity), #414 (WAL stream generation null end-to-end — the production defect motivating FR-006/User Story 4; open, Draft status as of this round).
