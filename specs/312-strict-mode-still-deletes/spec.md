...
This mirrors ADR-0310's reasoning, but is not assumed from it — see the two paragraphs above for why the same conclusion in fact follows independently for entities. The formal decision record for this issue is [`docs/adr/0312-entity-strict-mode-reclassifies-not-drops.md`](../../docs/adr/0312-entity-strict-mode-reclassifies-not-drops.md), written during the Plan/Implement stage per this repository's convention that ADR numbers are issue numbers (see `CLAUDE.md`); it must state this reasoning and cite ADR-0310 as precedent, not authority.
...
- The formal ADR ([`docs/adr/0312-entity-strict-mode-reclassifies-not-drops.md`](../../docs/adr/0312-entity-strict-mode-reclassifies-not-drops.md)) is written during the Plan/Implement stage per this repository's issue-numbered-ADR convention (`CLAUDE.md`); this spec records the decision and reasoning it must capture, not the ADR document itself.
