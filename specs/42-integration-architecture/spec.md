...
2. The **renderer's ContextGraphPanel** polls `knowledge_status` through the same `mcpDirectInvoker` → reader MCP path. During heavy reads (e.g., agent's `search_passages`), the renderer's status poll queues at the MCP transport even though the underlying DB read would be instant.
...
The ContextGraphPanel polls `knowledge_status` every few seconds. While the indexing queue processes a 100-chunk backlog (3+ hours of writes), the panel should still render fresh counts within 10ms p95.
...
3. **Given** the bundled binary is missing or corrupt, **When** the app starts the context graph service, **Then** the error surfaces clearly to the user with a recovery hint, not a silent failure.
...
#### `ContextGraphSocketClient` (TS, in liminis-app/src/main)

- **FR-001**: New module `context-graph-socket-client.ts` providing a typed wrapper around the JSON-RPC-over-Unix-socket protocol the Rust binary speaks.
...
- **FR-006**: `ContextGraphSocketClient.rebuildFromWal(progressCallback)` accepts a callback, parses `{type: "progress", ...}` lines off the socket, invokes the callback per line, and resolves with the terminal response. This method runs on the `lifecycle` connection and is not exposed as an MCP tool.
...
- **FR-009**: Each provider's tool handlers call `ContextGraphSocketClient` methods on dedicated `agent-read` and `agent-write` connections. No shared in-flight state across providers.
...
- **FR-012**: `mcpDirectInvoker` no longer handles `knowledge-*` calls. Audit `liminis-app/src/main/` for any remaining `mcpDirectInvoker.callTool(KNOWLEDGE_*, ...)` call sites and migrate each to `contextGraphClient.*` direct calls.

#### Indexing Queue Refactor

- **FR-013**: `liminis-app/src/main/indexing-queue.ts:1481` migrates from `mcpDirectInvoker.callTool('knowledge-writer', 'knowledge_process_chunk', args)` to `contextGraphClient.processChunk(args)` on the `indexing-write` connection.
...
- **FR-016**: All handlers in `liminis-app/src/main/ipc/context-graph-handlers.ts` migrate from `mcpDirectInvoker.callTool(KNOWLEDGE_*, ...)` to `contextGraphClient.*` on `renderer-read` / `renderer-write` connections.
- **FR-017**: ContextGraphPanel polling behavior is unchanged from the renderer's perspective.

#### Lifecycle Refactor

- **FR-018**: `context-graph-lifecycle.ts` health-probes use `contextGraphClient.healthCheck()` on the `lifecycle` connection rather than spawning probe subprocesses or mcpDirectInvoker calls.
- **FR-019**: `prepare_checkpoint` (called from `workspace-checkpoint.ts`) uses `contextGraphClient.prepareCheckpoint()` direct, not via MCP.
...
- `knowledge_rebuild_from_wal` is an admin operation, not an agent tool. It is not exposed via MCP; access is exclusively via `ContextGraphSocketClient` on the `lifecycle` connection.
...
- Removal of the upstream Python graphiti-core service, `reader_server.py`, `writer_server.py` from liminis-framework. (Deprecate first; remove in a follow-up release after one or more workspaces have validated the bundled path.)
...
- `liminis-app/src/main/ipc/context-graph-handlers.ts` — renderer IPC handlers to migrate
- `liminis-app/src/main/context-graph-lifecycle.ts` — lifecycle manager; binary path resolution to migrate
