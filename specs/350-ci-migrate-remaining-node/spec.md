...
- **FR-002**: For each of the five actions, exactly one version MUST be used across every call site in the repository — no file may pin a different major version of the same action than any other file.
...
- The target Node 24 releases are the same ones already in use elsewhere in the repo for the actions that have already migrated: `actions/checkout@v6`, `actions/upload-artifact@v7`, `actions/download-artifact@v8`. For `actions/cache` and `actions/cache/restore` (not yet used anywhere in this repo at a Node 24 version), the target is `actions/cache@v5` / `actions/cache/restore@v5` — `actions/cache`'s v5 release is the Node 24 release, and `cache`/`cache/restore`/`cache/save` ship from the same repository and share a version line.
