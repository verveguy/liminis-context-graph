#!/usr/bin/env bash
# Pre-build gate. Validates that the embedding assets produced by
# `./prepare-embedding-assets.sh` are present AND non-empty, so a half-populated
# directory cannot slip through into a broken build.
...
FIX_HINT="Run: ./prepare-embedding-assets.sh (from native/local-inference/)"
...
MLPACKAGE="$SIDECAR_ROOT/bge-base-en-v1.5.mlpackage"
...
TOK_DIR="$REPO_ROOT/resources/models/tokenizer/models/BAAI/bge-base-en-v1.5"
...
[ -d "$METADATA_DIR" ] || fail "missing tokenizer metadata directory: $METADATA_DIR (re-run ./prepare-embedding-assets.sh)"
for f in tokenizer.json tokenizer_config.json vocab.txt special_tokens_map.json config.json; do
  [ -s "$METADATA_DIR/${f}.metadata" ] || fail "missing or empty .metadata marker for $f in $METADATA_DIR (re-run ./prepare-embedding-assets.sh)"
done
