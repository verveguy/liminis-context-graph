...
Left alone this makes the release binary depend on a dynamic `libssl`/`libcrypto`, and on macOS it bakes in Homebrew's absolute install name (`/opt/homebrew/opt/openssl@3/lib/libssl.3.dylib`), so the artifact will not load for anyone without that exact path — Intel Homebrew, MacPorts, or no package manager. OpenSSL is linked **dynamically on purpose** (the user's package manager supplies it, so their CVE updates reach it; upstream states the same position in LadybugDB/ladybug#681). What must not ship is a binary naming a specific machine's OpenSSL. `scripts/stage-openssl-rpath.sh` prevents that by linking against dylib copies whose `install_name` has been rewritten to `@rpath/...`, and embedding the Homebrew ARM / Homebrew Intel / MacPorts roots as `LC_RPATH`; `scripts/assert-openssl-linkage.sh` is the guard that fails the build if any OpenSSL reference is an absolute path (macOS) or a path rather than a bare SONAME (Linux). Linux needs no staging — ELF records a SONAME that `ld.so` resolves at load time — so the staging script is a no-op there. **Both scripts run in both workflows** — in `ci.yml`'s `build-release`, and *separately* in `release.yml`, because cargo-dist inlines `.github/build-setup.yml` at generate time rather than referencing it at run time. The `release.yml` assertion is the one that matters, since it is the only one that inspects the artifacts that ship: it runs after `dist build`, before upload, over every executable sitting directly in a `target/<triple>/dist/` directory. It matches **by position, not by name**, because cargo-dist publishes *two* binaries — `liminis-context-graph` (from `lcg-service`) and `lcg-eval` — and `lcg-eval` reaches lbug via `lcg-core`, so it links OpenSSL on identical terms. In `release.yml` the *staging* step's position is load-bearing: it must sit **after** cargo-dist's `Install dependencies` step, which installs the `openssl@3` keg the script copies from — which is also why it is deliberately absent from `build-setup.yml`, a fragment cargo-dist always inlines *above* that step. The rewrite happens at **link time**, not by `install_name_tool` on the finished binary, because `dist build` builds and packages in one step — a post-build rewrite would leave the tarball holding the un-rewritten binary. See **ADR-0550** (which supersedes ADR-0398). Local `cargo build` without the script bakes in the local Homebrew path, which is fine for development.

**`.cargo/config.toml` pins `LBUG_VERSION = "0.20.2"`, and that pin is load-bearing.** It must always match the `lbug` crate pin in the workspace `Cargo.toml` — the two move together or not at all. Without it `build.rs` downloads the *floating* `latest` ladybug release, building the pinned crate's FFI against a mismatched native bundle. That skew broke the v0.9.0 release: the then-latest 0.18.1 bundle linked httplib against OpenSSL while the pinned 0.17.0 crate expected the bundled mbedtls, and the macOS build failed with `ld: symbol(s) not found`. The same file sets `RUST_TEST_THREADS = "4"` because lbug mmaps an 8 TB virtual region per `Db::open()` and default parallelism exhausts the macOS VM ceiling.
