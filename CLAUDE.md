...
Left alone this makes the release binary depend on a dynamic `libssl`/`libcrypto`, and on macOS it bakes in Homebrew's absolute install name (`/opt/homebrew/opt/openssl@3/lib/libssl.3.dylib`), so the artifact will not load for anyone without that exact path. `scripts/stage-openssl-static.sh` prevents this by pointing `PKG_CONFIG_PATH` at a directory holding only `libssl.a`/`libcrypto.a`; `scripts/assert-static-openssl.sh` is the guard that fails the build when a dynamic OpenSSL dependency survives anyway. **Both scripts run in both workflows** — in `ci.yml`'s `build-release`, and *separately* in `release.yml`, because cargo-dist inlines `.github/build-setup.yml` at generate time rather than referencing it at run time. The `release.yml` assertion is the one FR-009 actually rests on, since it is the only one that inspects the artifacts that ship: it runs after `dist build`, before upload, over every executable sitting directly in a `target/<triple>/dist/` directory. It matches **by position, not by name**, because cargo-dist publishes *two* binaries — `liminis-context-graph` (from `lcg-service`) and `lcg-eval`, each with its own per-target tarballs and installer — and `lcg-eval` reaches lbug via `lcg-core`, so it links OpenSSL on identical terms. A name filter would inspect one and let the other ship unchecked; position-matching also covers a future third binary without editing the step. In `release.yml` the *staging* step's position is load-bearing: it must sit **after** cargo-dist's `Install dependencies` step, which is what installs the `libssl-dev` that supplies the `.a` archives the script requires — which is also why it is deliberately absent from `build-setup.yml`, a fragment cargo-dist always inlines *above* that step. See **ADR-0398**. Local `cargo build` without the script links dynamically, which is fine for development.
