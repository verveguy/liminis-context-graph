    ...
    vec![
        ...
        ToolSpec {
            ...
            description: "Get knowledge graph status: entity/episode/relationship counts, \
                           ...
                           indices are built. Always returns a status object, never a JSON-RPC \
                           error, even when the database is open but a core table is broken — \
                           check the `queryable` field (and `reason` when false) to distinguish \
                           that state from a genuinely empty graph, whose counts read as 0 \
                           rather than null.",
            scope: Scope::Read,
