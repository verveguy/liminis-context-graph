...
async fn clear_group_for_rebuild(state: &Arc<AppState>, group_id: &str) -> Result<(), Error> {
    ...
    tokio::task::spawn_blocking(move || -> Result<(), Error> {
        ...
        let seq = wal_exec::wal_flush_ungrouped(&state_c, DEFAULT_GROUP_ID, conn.drain_mutations());
        // Persist DEFAULT_GROUP_ID's advanced position first — before the unconditional reset
        // below — so that in the coincidental case gid == DEFAULT_GROUP_ID, the intentional
        // "this group is now empty" reset always wins over the flush's own advance (issue #383).
        wal_exec::advance_applied_seq(&conn, DEFAULT_GROUP_ID, seq);
