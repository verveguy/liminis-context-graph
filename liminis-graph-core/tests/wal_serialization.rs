...
fn test_walline_deserializes_python_fixture() {
    let content = std::fs::read_to_string(fixture_path("python_produced.jsonl"))
        .expect("read fixture");
    ...
    for raw in content.lines() {
        ...
        assert_eq!(
            line.seq, expected_seqs[idx],
            "seq mismatch at line {idx}"
        );
        idx += 1;
    }
    assert_eq!(idx, expected_seqs.len(), "expected {} lines", expected_seqs.len());
}
