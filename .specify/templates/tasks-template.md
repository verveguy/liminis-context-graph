...
- **[Constitution-tag]** (per liminis-graph constitution v1.0.0):
  ...
  - `[ADAPTER]` — touches an LLM or embedding adapter; out-of-process boundary MUST be preserved
...
- `[IPC]` / `[LDB]`: parity test runs in CI against the recorded Python request/response corpus
- `[WAL]`: TDD log shows the failing test before the implementation commit
- `[HOT]`: a bench in `benches/` exists and the Performance & Resource Budget for that path is met (see plan.md Constitution Check)
- `[ADAPTER]`: no MLX, sentence-transformers, or other ML-runtime dependency added to `Cargo.toml`

A task that cannot meet its gate MUST surface an ADR draft in `docs/adr/` or escalate to a constitution amendment before merging.
